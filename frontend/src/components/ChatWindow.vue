...
import {streamAgentChat, getMessages} from '../api/chat.js'
...
const emit = defineEmits(['new', 'created', 'refreshed', 'session-expired'])
...
const loading = computed(() => {
  const id = props.session?.id || 'temp'
...
function checkIsExpired(e) {
  if (e?.response?.status === 404) return true;
  if (e?.code === 404 || e?.data?.code === 404 || e?.response?.data?.code === 404) return true;
  const msg = e?.message || e?.response?.data?.message || e?.data?.message || '';
  if (msg.includes('404') || msg.includes('会话不存在')) return true;
  return false;
}
...
watch(
    ...
    async (id) => {
      ...
      if (!sessionCache.value[id]) {
        ...
        try {
          let payload = await getMessages(id)
          if (payload.data && (payload.code === 200 || payload.code === 0)) {
          ...
          if (checkIsExpired(e)) {
            delete sessionCache.value[id]
            emit('session-expired', id)
          }
        }
      } else {
        // 如果有缓存，后台静默刷新以保证与数据库完全一致
        if (!abortControllers.has(id)) {
          try {
            let payload = await getMessages(id)
            if (payload.data && (payload.code === 200 || payload.code === 0)) {
...
async function send() {
  ...
  // 1. 本地乐观 UI 更新：先把消息推入视图，给用户秒回的体验
  if (!currentSessionId) {
    tempMessages.value.push({role: 'user', content: text})
    tempMessages.value.push({role: 'assistant', content: '', _flag: uniqueId})
  } else {
  ...
  const requestKey = currentSessionId || 'temp'
  ...
  try {
    await streamAgentChat({
      ...
      onToolCall: (toolName, args) => {
        ...
        const targetMsg = targetList.find(m => m._flag === uniqueId)
        if (targetMsg) {
          targetMsg._status = '🔧 正在调用工具：' + toolName
          scrollToBottom()
        }
      },
      onToolResult: (toolName, result) => {
        ...
        const targetMsg = targetList.find(m => m._flag === uniqueId)
        if (targetMsg) {
          targetMsg._status = '✅ 工具 ' + toolName + ' 已返回，继续分析...'
          scrollToBottom()
        }
      },
      onDone: (fullReply, newSessionId) => {
        const targetList = currentSessionId
            ? sessionCache.value[currentSessionId]
            : tempMessages.value
        if (targetList) {
        ...
        if (!currentSessionId && newSessionId) {
          sessionCache.value = {
            ...sessionCache.value,
            [newSessionId]: [...tempMessages.value]
          }
          emit('created', {id: newSessionId, title: text.substring(0, 15)})
        }
      }
    })

  } catch (e) {
    let errorMsg = '⚠️ ' + (e.message || '对话出错，请稍后重试')
    if (checkIsExpired(e)) {
      errorMsg = '⚠️ 当前会话已过期（后端服务重启），请开启新对话。'
      if (currentSessionId) {
        emit('session-expired', currentSessionId)
      }
    }
    ...
    <div class="message-list" ref="listRef">
      <div v-if="messages.length === 0" class="empty-state">
        ...
        <div class="suggest-cards">
          <button class="suggest-card" @click="input = '帮我写一个 Java 的冒泡排序示例'; send()">
            <span class="suggest-icon">💻</span> 写一段代码
          </button>
          <button class="suggest-card" @click="input = '解释一下什么是 RESTful API，并举例子'; send()">
            <span class="suggest-icon">📚</span> 解释概念
          </button>
          <button class="suggest-card" @click="input = '帮我制定一个学习 Spring Boot 的计划'; send()">
